...
export function createEarthGroup(pos) {
  ...
  const texture = textLoader.load("src/images/planets/earth.jpg");
  ...
  const earthLight = textLoader.load("src/images/planets/earth_light.jpg");
  ...
  const earthCloudTexture = textLoader.load("src/images/planets/earth_clouds.jpg");
...
export function getPlanet({img = '', size = 1, position = [], glow}) {
  ...
  const map = textLoader.load(`src/images/planets/${img}`);
...
export function getSaturnsRing({img = '', position = []}) {
  const ringTexture = textLoader.load(`src/images/planets/${img}`);
